...
const MediaTab = ({ isAuthenticated, username }: IProps) => {
  const { ref, inView } = useInView();

  const fetchUserTweets = ({ pageParam = 0 }) => {
    return getMediaOnlyUserTweets(username, pageParam, 20);
  };
  ...
  const {
    ...
    isFetchingNextPage,
    refetch,
  } = useInfiniteQuery(["mediaOnlyUserTweets", username], fetchUserTweets, {
          ...
          page.data.length > 0 ? (
            ...
            <div key={index} className="flex p-8 justify-center">
              <div className="flex flex-col max-w-sm">
                ...
                <span className="text-[color:var(--color-base-secondary)]">When they do, their Tweets will show up here.</span>
              </div>
            </div>
          )
        )}
